  ...
  "))
    ...
    tabItems(
      ...
      tabItem(tabName = "model_fitting",
              h2("Model Fitting"),
              actionButton("run_model", "Run Bayesian GLM")
      ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$saidi_file, {
    ...
    output$diagnostic_plots <- renderPlot({
      models <- model_results()
      if (!is.null(models)) {
        # Set up the plot layout (e.g., 3 rows, 2 columns)
        par(mfrow = c(3, 2))
        
        # Diagnostic plots for SAIDI Spline
        plot(models$saidi_spline, main = "SAIDI Spline Diagnostics")
        
        # Diagnostic plots for SAIFI Spline
        plot(models$saifi_spline, main = "SAIFI Spline Diagnostics")
        
        # Diagnostic plots for EENS Spline
        plot(models$eens_spline, main = "EENS Spline Diagnostics")
        
        # Diagnostic plots for SAIDI Linear
        plot(models$saidi_linear, main = "SAIDI Linear Diagnostics")
        
        # Diagnostic plots for SAIFI Linear
        plot(models$saifi_linear, main = "SAIFI Linear Diagnostics")
        
        # Diagnostic plots for EENS Linear
        plot(models$eens_linear, main = "EENS Linear Diagnostics")
        
      } else {
        plot(1, type = "n", axes = FALSE, xlab = "", ylab = "",
             main = "Diagnostic plots failed. Check data and model specification.")
      }
    })
    ...
    output$diagnostic_plots <- renderPlot({
      models <- model_results()
      if (!is.null(models)) {
        # Use bayesplot functions which are static (no "Hit Return" prompt)
        # We create a grid of trace plots for the Spline models
        
        # 1. SAIDI Trace Plot
        p1 <- mcmc_trace(as.array(models$saidi_spline), 
                         pars = c("b_Intercept", "sds_sRESPenetrationLevel_1")) + 
          ggtitle("SAIDI Trace Plot")
        
        # 2. SAIFI Trace Plot
        p2 <- mcmc_trace(as.array(models$saifi_spline), 
                         pars = c("b_Intercept", "sds_sRESPenetrationLevel_1")) + 
          ggtitle("SAIFI Trace Plot")
        
        # 3. EENS Trace Plot
        p3 <- mcmc_trace(as.array(models$eens_spline), 
                         pars = c("b_Intercept", "sds_sRESPenetrationLevel_1")) + 
          ggtitle("EENS Trace Plot")
        
        # Combine them into one grid (requires gridExtra package or simply bayesplot_grid)
        bayesplot_grid(p1, p2, p3, grid_args = list(ncol = 1))
